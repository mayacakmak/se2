...
ROS3D.Arrow = function(options) {
  ...
  m1.setPosition(new THREE.Vector3(0, shaftLength * 0.5, 0));
  geometry.applyMatrix(m1);

  // create the head
  var m2 = new THREE.Matrix4();
  var coneGeometry = new THREE.CylinderGeometry(0, headDiameter * 0.5, headLength, 12, 1);
  m2.setPosition(new THREE.Vector3(0, shaftLength + (headLength * 0.5), 0));
  coneGeometry.applyMatrix(m2);

  // put the arrow together
  var m3 = new THREE.Matrix4();
  THREE.GeometryUtils.merge(geometry, coneGeometry);
  //geometry.merge(coneGeometry, m1.multiply(m3.getInverse(m2)));
  //geometry.merge(coneGeometry, m2.multiply(m3.getInverse(m1)));
  //geometry.merge(coneGeometry, m1);
  //geometry.merge(coneGeometry, m2);
...
ROS3D.Viewer = function(options) {
  ...
  function draw() {
    ...
    that.directionalLight.matrixWorldNeedsUpdate = true;
...
ROS3D.Highlighter.prototype.onMouseOver = function(event) {
  ...
  var hoverObj = event.intersection.object;
  if (hoverObj.material) {
      hoverObj.__old_material = hoverObj.material;
      hoverObj.material = new THREE.MeshBasicMaterial({
          fog : false,
          opacity : 0.5,
          depthTest : true,
          depthWrite : false,
      ...
      hoverObj.material.needsUpdate = true;
  }
};
...
ROS3D.Highlighter.prototype.onMouseOut = function(event) {
  ...
  //this.hoverObjs = [];
  var hoverObj = event.intersection.object;
  if (hoverObj.__old_material) {
    hoverObj.material = hoverObj.__old_material;
    hoverObj.material.needsUpdate = true;
  }
};
...
//  scene.__webglObjects = renderList;
  //var renderList  = this.hoverObjs;
  //scene.traverse(function (obj) { if (obj.children.length === 0 && obj.type.slice(-5) !== 'Light' && renderList.indexOf(obj) === -1) { obj.visible = false; } });
//  renderer.render(scene, camera);

  //scene.traverse(function (obj) { obj.visible = true; });
